...
class RequestService:
    ...
    def create_new_request(self, address, name, phone, source, coordinates, author_id):
        ...
        db.session.commit()
        self.logging.add_log(int(new_request.id),
                             f'Заявка {int(new_request.id)} создана пользователем {current_user.username}')
        return
    ...
    def get_all(self, page=1):
        return Requests.query.paginate(page=page, per_page=2, error_out=True)
    ...
    def get_request_dataset(self, address, name, phone, source, base, \
                            device, status, start_date, end_date, page):
        ...
        if base:
            request_dataset = request_dataset.filter(Requests.base.ilike(f'%{base}%'))

        return request_dataset.paginate(page=page, per_page=2, error_out=True)
